...
function calculateCartCostAndQuantity(items: CartItem[]) {
    ...
    }, { totalQuantity: 0, cost: { totalCost: 0, subtotalCost: 0, totalTaxAmount: 0 } });
}
...
function updateCart(cartState: Cart | undefined, cartAction: CartAction): Cart {
    ...
    switch (cartAction.type) {
        case "ADD_TO_CART": {
            ...
            const existingItem = cart.items.find(item => item.merchandise.id === variant.id);
            ...
            const updatedItems = cart.items
                .map(item => item.merchandise.id === existingItem.merchandise.id ? updateCartItem(existingItem, "plus") : item) as CartItem[];
            return {
                ...cart,
                ...calculateCartCostAndQuantity(updatedItems),
                items: updatedItems
        ...
        case "UPDATE_CART": {
            ...
            const newItems = cart.items
                .map(item => item.merchandise.id === merchandiseId ? updateCartItem(item, action) : item)
